  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.penultimate-guitar-migrate = lib.mkIf cfg.postgresql.enable {
      ...
      serviceConfig = {
        ...
        ExecStart = "${pkgs.nodejs_20}/bin/node ${cfg.package}/node_modules/.bin/prisma migrate deploy --schema=${cfg.package}/prisma/schema.prisma";
    ...
    systemd.services.penultimate-guitar = {
      ...
      serviceConfig = {
        ExecStart = "${pkgs.nodejs_20}/bin/node ${cfg.package}/server.js";
        WorkingDirectory = cfg.package;
        EnvironmentFile = cfg.environmentFile;
        DynamicUser = true;
        PrivateTmp = true;
